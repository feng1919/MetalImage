/*
 ...
 Abstract:
 A functor for performing matrix multiplication using BLAS sgemm method
 */
...
- (instancetype) init
{
    ...
    if(self)
    {
        ...
        dispatch_once(&token, ^{
            m_DQueue = dispatch_queue_create("com.apple.matrixmult.blas.main", DISPATCH_QUEUE_CONCURRENT);
        });
             ...
             with:(float*)matB
{
    if((matA != nullptr) && (matB != nullptr))
    {
        ...
        __block float* pMatA = nullptr;
        __block float* pMatB = nullptr;
        ...
        if(mpOutput != nullptr)
        {
            ...
            dispatch_async(m_DQueue, ^{
                cblas_sgemm(_order,     // Specifies row-major or column-major data ordering
